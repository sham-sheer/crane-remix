...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly create crane-7df4
  fly create crane-7df4-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app crane-7df4
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app crane-7df4-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'crane-7df4-staging' does not match app name in config file 'crane-7df4'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name crane-7df4-db
  fly postgres attach --postgres-app crane-7df4-db --app crane-7df4

  fly postgres create --name crane-7df4-staging-db
  fly postgres attach --postgres-app crane-7df4-staging-db --app crane-7df4-staging
  ```
